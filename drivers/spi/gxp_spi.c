...
#ifdef CONFIG_SPI_CMD
// the spi driver is only for mtd spi_flash using. 
// it doesn't support spi command 
#error gxp_spi.c do not support CONFIG_SPI_CMD
#endif
...
//mode = 1, auto 
	...
	if(mode==MANUAL_MODE) {
		...
		value &= ~(0x03<<4); // clear bit5 and bit4, auto_start and start_mask
	}
	else {
		value |= (0x03<<4);
	}
...
static int gxp_spi_xfer(struct udevice *dev, unsigned int bitlen, const void *dout, void *din, unsigned long flags)
{
	...
	if(dout && din) {
		//error: gxp spi engin cannot send data to dout and read data from din at the same time
		return -1;
	}
	...
	if (flags & SPI_XFER_BEGIN) {
		...
		value &= ~(1<<24);
		value |= (slave_plat->cs<<24);	//set chipselect
		...
		if(len >= 4) {
			...
			value &= ~(0x07<<16);
			value |= (3<<16);	//set the address size to 3 byte
		}
		else {
			writel(0, priv->base + OFFSET_SPIADDR);
			value &= ~(0x07<<16);	// set the address size to 0 byte
		}

		//dummy
		value &= ~(0x1f<<19); // clear dummy_cnt to 0
		if(uchar_out[0] == CMD_READ_ARRAY_FAST) {
			value |= (8<<19);  //fast read needs 8 dummy clocks
		}
		...
		if(flags & SPI_XFER_END) {
			...
			// set the start bit
			value = readb(priv->base + OFFSET_SPIMCTRL);
			value |= SPIMCTRL_START;		//set the start bit
			writeb(value, priv->base + OFFSET_SPIMCTRL);
