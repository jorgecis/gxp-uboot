...
static int gxp_umac_init(struct eth_device *dev, bd_t * bis)
{
	int ret;
	struct gxp_umac_private *priv;
	struct phy_device *phydev;
...
static int gxp_umac_ll_register(bd_t * bis, struct mii_dev *external_mdio_bus, int external_dev_num,
 ...
 */
int gxp_umac_register(bd_t * bis)
{
	int rc;
	int init_count = 0;
